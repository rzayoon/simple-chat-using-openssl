...
void init_OpenSSL(void){
	if(!THREAD_setup() || !SSL_library_init()){
		fprintf(stderr, "** Open SSL initialization failed! \n");
		exit(-1);
	}
...
int verify_callback(int ok, X509_STORE_CTX *store){
	...
	if (!ok){
		...
		fprintf(stderr, "-Error wit certificate at depth: %i\n", depth);
...
#else
				ext_str = meth->d2i(NULL, (const unsigned **)&data, ext->value->length);
#endif
